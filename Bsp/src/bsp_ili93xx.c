...
void LCD_DrawBMP( uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1 , uint16_t *bmp )
{
  ...
  int i; 
	...
	uint16_t *pcolor;
	pcolor = bmp;
	LCD_SetWindows(x0,y0,x1,y1);//窗口设置
    for(i=0;i<104*104;i++)
	{	
	 	//picL=*(p+i*2);	//数据低位在前
		//picH=*(p+i*2+1);				
...
{  
	...
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				if(mode&0X80)LCD_ShowChar(x+(size/2)*t,y,POINT_COLOR,BACK_COLOR,'0',size,mode&0X01);  
				else LCD_ShowChar(x+(size/2)*t,y,POINT_COLOR,BACK_COLOR,' ',size,mode&0X01);  
 				continue;
			}else enshow=1; 
		 	 
		}
	 	LCD_ShowChar(x+(size/2)*t,y,POINT_COLOR,BACK_COLOR,temp+'0',size,mode&0X01); 
	}
} 
//******************************************************************
//函数名：  LCD_ShowString
...
void GUI_DrawFont16(u16 x, u16 y, u16 fc, u16 bc, u8 *s,u8 mode)
{
	...
	u16 HZnum;
	u16 x0=x;
	HZnum=sizeof(tfont16)/sizeof(typFNT_GB16);	//×??ˉí3??oo×?êy??
	  ...
	  { 	LCD_SetWindows(x,y,x+16-1,y+16-1);
		    for(i=0;i<16*2;i++)
		    {
		    	...
		    	{	
					...
					{
						POINT_COLOR=fc;
						if(tfont16[k].Msk[i]&(0x80>>j))	LCD_DrawPoint(x,y);//?-ò???μ?
						x++;
						if((x-x0)==16)
						{
							x=x0;
	...
	LCD_SetWindows(0,0,lcddev.width-1,lcddev.height-1);//???′′°?ú?aè??á  
} 
