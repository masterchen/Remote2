...
{
	if ((_usX <= THRESHOLD) || (_usY <= THRESHOLD)
		|| (_usX >= 4095 - THRESHOLD)
		|| (_usY >= 4095 - THRESHOLD))
	{
		return 0;
...
void TOUCH_Scan(void)
{
  ...
  static uint16_t s_usXBuf[SAMPLE_COUNT];
	...
	usAdcX=XPT2046_ReadAdc(CHX);
	usAdcY=XPT2046_ReadAdc(CHY);
		...
		if (s_count >= 30)
		{
			...
			if (++s_ucPos >= SAMPLE_COUNT)
			{
				s_ucPos = 0;
			   	...
			   	/* 对ADC采样值进行软件滤波 */
				NowPtr.x = TOUCH_DataFilter(s_usXBuf, SAMPLE_COUNT);
				NowPtr.y = TOUCH_DataFilter(s_usYBuf, SAMPLE_COUNT);
			  ScreenPtr.x = ( (aa1 *NowPtr.x) + (bb1 * NowPtr.y) + cc1);        
        ScreenPtr.y = ((aa2 * NowPtr.x) + (bb2 * NowPtr.y) + cc2 );
